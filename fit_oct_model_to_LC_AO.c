...
void fit_oct_model_to_LC_AO(LCstruct *LC,AOstruct *AO,OCstruct *OC,RDstruct *RD,CNTRstruct *CR)
{
    ...
    if(INI_FIT_ALBEDO==1)
    {
        ...
        nAlbreg=nAlbedo;
        if(INI_ALBEDO_FILE!=NULL)
        {
            int read_alb=0;
            read_alb=read_vector_file(INI_ALBEDO_FILE,eAlbedo,nAlbedo);
            if(read_alb<nAlbedo)
            {
                printf("Wrong number of albedo values in %s\n",INI_ALBEDO_FILE);
    ...
    double *inerdv=NULL,*inerda=NULL;
    if(INI_INER_WEIGHT>0)
    {
        inerreglength=1;
        inerdv=calloc(3*nvert+3,sizeof(double));
        inerda=calloc(alength+3,sizeof(double));
    }
    //Number of rows in J
    ...
    Slength=nLCtotal+nAOtotal+nOCtotal+nRDtotal+1+1+1+softmaxz+nvectorreg+nAlbreg+nCRtotal+nCOM+inerreglength; //LC points+AO points+OC points+RD points+ convex reg+oct reg+dihedral+[softmaxz]+[vector reg for free chords]
    ...
    if(INI_RESTORE_STATE)
    {
        ...
        //Restore albedo
        int albcount=0;
        if(INI_HAVE_LC && INI_FIT_ALBEDO==1 && INI_RESTORE_ALBEDO)
        {
            albcount=read_state_file(INI_RESTORE_STATE,"#Albedolog",eAlbedo,nAlbedo);
            printf("Restoring albedo values\n");
        if(albcount!=nAlbedo)
            {
    ...
    for(int k=0;k<NUM_OF_ROUNDS;k++)
    {
        ...
        if(decreased==1)
        {
            ...
            if(INI_INER_WEIGHT>0)
            {
                ...
                inertia(tlist,vlist,nfac,nvert,NULL,0,0,&inerres,&inerangle,inerdv);
                printf("Inerres: %f Inerangle: %f\n",inerres,inerangle);
               
                matrix_prod(inerdv,1,3*nvert+3,D1,alength+3,inerda);
                inerres*=INI_INER_WEIGHT;
                mult_with_cons(inerda,1,alength+3,-INI_INER_WEIGHT);
                S[inerregpos]=inerres;
                set_submatrix(J,Slength,nJcols,inerda,1,alength,inerregpos,0);
            }
            ...
            if(fidlog!=NULL)
            {
               fprintf(fidlog,"Round: %d chisq: %4.2f LCfit: %4.2f AOfit: %4.2f OCfit: %4.2f RDfit: %4.2f Convex reg: %4.2f Octantoid reg: %4.2f Dihedral reg: %4.2f",k,chisq,LCfit,AOfit,OCfit,RDfit,pow(CRres,2),pow(Ores,2),pow(ANGres,2)); 
            //printf("scale: %f %f\n",AOscale[0],AOscale[1]);
            //Construct Jacobian matrix
            if(INI_HAVE_CNTR>0)
                fprintf(fidlog," CNTRfit: %4.2f",CRfit);
            if(INI_COM_WEIGHT>0)
                fprintf(fidlog," CoM reg: %4.2f",pow(CoM,2));
            if(INI_INER_WEIGHT>0)
                fprintf(fidlog," Iner reg: %4.2f",pow(inerres,2));
            if(INI_ZMAX_WEIGHT>0)
                fprintf(fidlog," Zmax reg: %f\n",pow(zm,2));
            else
                fprintf(fidlog,"\n");
            }
        ...
        if(info>0 && INI_HAVE_LC && INI_FIT_ALBEDO && first_time==1)
        {
            printf("Solving the matrix failed, fixing the albedo\n");
            if(fidlog!=NULL)
                fprintf(fidlog,"Solving the matrix failed, fixing the albedo\n");
            first_time=0;
            INI_MASK_SET=1;
        ...
        if(INI_INER_WEIGHT>0)
            {
                ...
                inertia(tlist,vlist2,nfac,nvert,NULL,0,0,&inerres,&inerangle,inerdv);
                inerres*=INI_INER_WEIGHT;
                S[inerregpos]=inerres;
            }
        ...
        if(fidlog!=NULL)
            fprintf(fidlog,"Round: %d chisq2: %7.2f\n",k,chisq2);
        if(chisq2<chisq)
        {
            prevdec=dec;
            dec=chisq-chisq2;
        ...
        if(prevdec<threshold && dec<threshold)
        {
            ...
            if(fidlog!=NULL)
                fprintf(fidlog,"Two previous optimization steps below threshold, stopping optimization loop.\n");
            DONE=1;
        }
        if(lambda>INI_LAMBDAMAX)
        {
            ...
            if(fidlog!=NULL)
                fprintf(fidlog,"lambda is larger than LambdaMax, halting optimization\n");
            DONE=1;
        }
        if(k==NUM_OF_ROUNDS-1 || DONE==1)
        {
            ...
            if(fidlog!=NULL)
            {
                fprintf(fidlog,"Angles: %.4f %.4f %.8f\n",angles[0],angles[1],angles[2]);
                fprintf(fidlog,"Volume equivalent diameter: %4.3f\n",vol_eq_dia(tlist,vlist,nfac,nvert));
            }
            if(INI_HAVE_OC)
            {
            ...
            if(INI_WRITE_STATE_FILE!=NULL)
            {
                ...
                if(INI_FIT_ALBEDO==1)
                {
                    ...
                    for(int j=0;j<nfac;j++)
                        fprintf(fp,"%.2f ",(Alblimits[0]+Alblimits[1])/2.0+(Alblimits[1]-Alblimits[0])/2.0*(tanh(eAlbedo[tlist[3*j]-1])+tanh(eAlbedo[tlist[3*j+1]-1])+tanh(eAlbedo[tlist[3*j+2]-1]))/3);
                    fprintf(fp,"\n");
            ...
            if(INI_FIT_ALBEDO==1 && INI_ALBEDO_OUT_FILE!=NULL)
            {
                ...
                for(int j=0;j<nfac;j++)
                    falbedo[j]=(Alblimits[0]+Alblimits[1])/2.0+(Alblimits[1]-Alblimits[0])/2.0*(tanh(eAlbedo[tlist[3*j]-1])+tanh(eAlbedo[tlist[3*j+1]-1])+tanh(eAlbedo[tlist[3*j+2]-1]))/3;
